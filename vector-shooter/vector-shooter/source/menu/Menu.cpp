...
void MenuObject::Init(sf::Texture& textureOn,sf::Texture& textureOff,sf::Vector2f& position,std::string text)
{
	...
	height_ = static_cast<float>(textureOn_->getSize().y);
		...
		// Loading text
	font_.loadFromFile("source\\data\\fonts\\sub.ttf");
...
void MainMenu::Init(sf::RenderWindow& window,MainMenuState& state)
{
	...
	// Loading texture
	buttonOn_.loadFromFile("source\\data\\assets\\grunge\\standard-button-on.png");
	buttonOff_.loadFromFile("source\\data\\assets\\grunge\\standard-button-off.png");
	dialogBox_.loadFromFile("source\\data\\assets\\grunge\\dialog-box.png");
